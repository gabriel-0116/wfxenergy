...
export default function ResumoProjeto({ cliente, projeto, dadosPrecificacao, variante = "proposta" }: Props) {
  ...
  // 🎨 Cores por contexto
  const tituloCor = variante === "proposta" ? "text-purple-400" : "text-orange-400";

  return (
    <div className="grid grid-cols-2 gap-6">
      <div className="bg-[#1a1a1a] rounded-xl shadow-2xl p-6 space-y-2">
        <h2 className={`text-lg font-semibold ${tituloCor} mb-3 border-b border-gray-600 pb-2`}>
        ...
        <h2 className={`text-lg font-semibold ${tituloCor} mb-3 border-b border-gray-600 pb-2`}>
          <FontAwesomeIcon icon={faBolt} className="text-zinc-200 text-xl mr-2" />
          Consumo
        </h2>
        ...
        <h2 className={`text-lg font-semibold ${tituloCor} mb-3 border-b border-gray-600 pb-2`}>
          <FontAwesomeIcon icon={faRulerCombined} className="text-zinc-200 text-xl mr-2" />
        ...
        <h2 className={`text-lg font-semibold ${tituloCor} my-3 border-b border-gray-600 pb-2`}>
          <FontAwesomeIcon icon={faSackDollar} className="text-zinc-200 text-xl mr-2" />
          Quanto vou pagar?
        </h2>
      ...
      <div className="bg-[#1a1a1a] rounded-xl shadow-xl p-6 space-y-2">
        <h2 className={`text-lg font-semibold ${tituloCor} mb-3 border-b border-gray-600 pb-2`}>
          <FontAwesomeIcon icon={faSolarPanel} className="text-zinc-200 text-xl mr-2" />
          Sistema Solar
        </h2>
        ...
        <p><strong>Geração mensal:</strong> {
          projeto.modo === "manual"
            ? projeto.geracaoMensalManual ?? "---"
            : projeto.geracaoMensal ?? "---"
        } kWh</p>
        <p><strong>Geração diária:</strong> {
          projeto.modo === "manual"
        ...
        <p><strong>Excedente Unidade:</strong> {
          projeto.modo === "manual"
        ...
        {dadosPrecificacao && (
          <div>
            <h2 className={`text-lg font-semibold ${tituloCor} mb-3 border-b border-gray-600 pb-2`}>
              <FontAwesomeIcon icon={faMoneyBill} className="text-zinc-200 mr-2" />
              Resumo Financeiro
            </h2>
            <p>
  <strong>Valor do Kit Fotovoltaico:</strong>{" "}
  R${Number(dadosPrecificacao.kitFotovoltaico ?? 0).toFixed(2)}
</p>

            <p><strong>Forma de Pagamento:</strong> {gerarFormaPagamento(
            ...
            {dadosPrecificacao.financiamentoSelecionado && (
              <>
                <p><strong>Entrada:</strong> R$ {dadosPrecificacao.entrada?.toFixed(2) ?? "0,00"}</p>
                <p><strong>Parcelas:</strong> {dadosPrecificacao.financiamentoSelecionado.parcelas}x de R$ {
                  dadosPrecificacao.financiamentoSelecionado.valorParcela.toFixed(2)
                }</p>
              </>
            )}
          </div>
