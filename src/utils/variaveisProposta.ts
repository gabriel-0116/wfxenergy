export const variaveisProposta = [
  {
    nome: "[[nome_cliente]]",
    descricao: "Nome completo do cliente (ou razão social, se for PJ)",
  },
  {
    nome: "[[cpf]]",
    descricao: "CPF do cliente ou CNPJ (se PJ)",
  },
  {
    nome: "[[rg]]",
    descricao: "RG do cliente (apenas para pessoa física)",
  },
  {
    nome: "[[telefone]]",
    descricao: "Telefone do cliente",
  },
  {
    nome: "[[cidade]]",
    descricao: "Cidade do cliente",
  },
  {
    nome: "[[estado]]",
    descricao: "Estado do cliente",
  },
  {
    nome: "[[nome_projeto]]",
    descricao: "Nome do projeto",
  },
  {
    nome: "[[quantidade_placas]]",
    descricao: "Quantidade de placas solares utilizadas",
  },
  {
    nome: "[[potencia_placas]]",
    descricao: "Potência de cada placa (em Watts)",
  },
  {
    nome: "[[potencia_instalada]]",
    descricao: "Potência total instalada (kWp)",
  },
  {
    nome: "[[geracao_media]]",
    descricao: "Geração média mensal do sistema (kWh/mês)",
  },
  {
    nome: "[[estrutura]]",
    descricao: "Tipo de estrutura do projeto",
  },
  {
    nome: "[[area_necessaria]]",
    descricao: "Área mínima necessária para instalação (em m²)",
  },
  {
    nome: "[[valor_a_vista]]",
    descricao: "Valor total à vista",
  },
  {
    nome: "[[total_financiado]]",
    descricao: "Valor total do projeto com financiamento",
  },
  {
    nome: "[[forma_pagamento]]",
    descricao: "Forma de pagamento (à vista ou parcelado)",
  },
  {
    nome: "[[data_assinatura]]",
    descricao: "Data atual (formato brasileiro)",
  },
  {
    nome: "[[nome_cliente_assinatura]]",
    descricao: "Nome do cliente para assinatura",
  },
  {
    nome: "[[consumo_medio_mensal]]",
    descricao: "Consumo médio mensal do cliente",
  },
  {
    nome: "[[consumo_medio_diario]]",
    descricao: "Consumo médio diário do cliente",
  },
  {
    nome: "[[qtd_inversor_microinversor]]",
    descricao: "Quantidade de inversores ou microinversores",
  },
  {
    nome: "[[potencia_inversor_microinversor]]",
    descricao: "Potência total do(s) inversor(es) (em kWp)",
  },
  {
    nome: "[[inversor_microinversor]]",
    descricao: "Tipo de inversor utilizado (inversor ou microinversor)",
  },
];
